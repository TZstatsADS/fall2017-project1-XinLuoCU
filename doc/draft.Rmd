...
This report uses nomination, inaugration and farewell speeches of presidents and candidates and it pays more attention to the incumbent president Trump than other presidents.
...
#PART I: Comparing the  Inaugaration Speeches with Nomination Speeches
...
#axis(side = 4,at=0:4,labels =c(0,'10%','20%'))
```
...
#plot_word_cloud(3,1)#Roosevelt
plot_word_cloud(4,2)#Bush iraq war
```

The keywords selected for each speech is very accurate. For example, the keyword of Obama's nomination speech is his competitor in the election, "Mccain" and the "icy", "stroms" showed in his inaugration speech reflect the financial crisis just happened and indicate Obama's first big deal is resolving the problems financial crisis brought to the country.

Besides, from the word clouds, we can see the nomination speeches always have more words and these words are very specific, like the country names and people names, while Inaugration speeches intend to contain the "bigger" words. 

Generally speaking, in nomination speeches, speakers provide people clear plans and promises in order to receive supports, while the inaugration speech do not have that strong purpose. While making inaugration speeches, speakers are more likely to summarize the situation of the country and the his main claims, they would not give that much details to people under the limitation of time.
...
Compare to the other presidents, sentiments of Trump's inaugration speech is obviously different from his nomination and looks distinctive in all these speeches. His inaugration emphasizes the opstimistic sentiments like "anticipation", "joy" and "trust" while reduce the percentages of "anger", "disgust", "sadness" and "negative" to a very low level. Personally speaking, it has strong desire to encourage people by avoiding the negative feelings. 
...
join_list1<-merge(Bush1,Bush2,by="term",all = T)
...
join_list2<-merge(Bush2,Obama1,by="term",all = T)
...
join_list3<-merge(Obama1,Obama2,by="term",all = T)
...
join_list4<-merge(Obama2,Trump,by="term",all = T)
...
The shared key words of two inaugration speeches of Obama contain "jobs", "pay", "afford" and "crisis", indicating the 2007-2008 financial crisis that Obama encountered.  

In the right plot, "economy" and "companies" are mentioned by both Trump and Obama, so the Business is not only Obama's big deal, but also Trump's focus.
...
###What are the presidents' favorite short sentences? 
```{r,inclue=F}
...
short_sentences1<-short_sentences%>%
  ...
  summarise(
    ...
    fav_president=paste(names(which(table(President)==max_times)),collapse=";")
...
###Who is the president love short sentences most???
```{r,include=F}
...
The champion is Trump, each speech of him has 64 short sentences in average. The presidents we are familar with like Obama and George Bush love using short sentences in their speeches too.
...
##1. Build the Model
###(a) make DocumentTermMatrix with the snipets of sentences 
```{r}
...
###(b) Run 10 Topics LDA model using  Gibbs Sampling
```{r}
...
##Analysis
###(a).What are the Top Terms of Each Topic?
```{r}
top_terms <- tidy_lda %>%
  group_by(topic) %>%
  top_n(8, beta) %>%
...
But if we check the gamma values of the topic model, we will find the model doesn't perform well.
```{r}
